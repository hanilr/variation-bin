...
install: $(lib)
	...
	@sudo cp $(lib) /usr/include/vn
	@sudo chmod -x /usr/include/vn/$(lib)

	@echo ""
	@echo "\t$(green_fg)[$(blue_fg) ---------------------------------------- $(green_fg)] "
	@echo "\t$(green_fg)[ $(red_fg)$(text_underline)Variation Binary$(end_text)$(white_fg) successfully installed! $(green_fg)] "
	@echo "\t$(green_fg)[$(blue_fg) ---------------------------------------- $(green_fg)] "
	@echo "$(end_text)"

uninstall:
	@sudo rm /usr/include/vn/$(lib)
	@if [ "$(ls -A "/usr/include/vn/" 2> /dev/null)" = "" ]; then sudo rm -rf /usr/include/vn/; fi

	@echo ""
	@echo "\t$(green_fg)[$(blue_fg) ------------------------------------------ $(green_fg)] "
	@echo "\t$(green_fg)[ $(red_fg)$(text_strikethrough)Variation Binary$(end_text)$(white_fg) successfully uninstalled! $(green_fg)] "
	@echo "\t$(green_fg)[$(blue_fg) ------------------------------------------ $(green_fg)] "
	@echo "$(end_text)"
