...
void vn_bit_clean_range(enum Bin_S Bin_Size, struct Bin_T *Bin, int start_pos, int end_pos) {
    ...
    while (1) { // Assignment
        if (Bin_Size == 4) {
            Bin->bit_type.Bit4_T[i] = LOW;
        } else if (Bin_Size == 8) {
            Bin->bit_type.Bit8_T[i] = LOW;
        } else if (Bin_Size == 16) {
            Bin->bit_type.Bit16_T[i] = LOW;
        } else if (Bin_Size == 32) {
            Bin->bit_type.Bit32_T[i] = LOW;
        } else if (Bin_Size == 64) {
            Bin->bit_type.Bit64_T[i] = LOW;
        } else if (Bin_Size == 128) {
            Bin->bit_type.Bit128_T[i] = LOW;
        }
        
        i += 1;
        if (i == end_pos + 1) break;
    }
}

void vn_bit_clear(enum Bin_S Bin_Size, struct Bin_T *Bin) {
    vn_bit_clean_range(Bin_Size, Bin, 0, Bin_Size);
}
...
struct Bin_T vn_bit_get_range(enum Bin_S Bin_Size, enum Bin_S Range_Size, struct Bin_T InputBin, int start_pos, int end_pos) {
    ...
    while (1) { // Assignment
        ...
        i += 1;
        if (i == Range_Size || i == end_pos - start_pos) break;
    }
