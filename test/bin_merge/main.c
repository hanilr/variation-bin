...
int main(void) {
    ...
    enum Bin_S New_Bin_Size = 16;

    struct Bin_T BinFirst = vn_int_to_bin(Bin_Size, first);
    struct Bin_T BinSecond = vn_int_to_bin(Bin_Size, second);
    struct Bin_T BinResult1 = vn_merge_bin(Bin_Size, BinFirst, BinSecond);
    struct Bin_T BinResult2 = vn_merge_bin(Bin_Size, BinSecond, BinFirst);
