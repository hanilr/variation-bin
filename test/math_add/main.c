...
int main(void) {
    ...
    struct Bin_T FirstBin = vn_int_to_bin(Bin_Size, 12);
    struct Bin_T SecondBin = vn_int_to_bin(Bin_Size, 24);

    bin_printer(Bin_Size, FirstBin);
    bin_printer(Bin_Size, SecondBin);
    bin_printer(Bin_Size, vn_add(Bin_Size, FirstBin, SecondBin));
